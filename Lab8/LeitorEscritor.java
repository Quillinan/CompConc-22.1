...
class Integer {
  ...
  // Funcao chamado pela Thread1
  public static void add() {
    x++;
  }

  // Funcao chamado pela Thread2
  public static void type() {
    String type = x % 2 == 0 ? "par." : "impar.";
    System.out.println(x+" eh "+type);
  }

  // Funcao chamado pela Thread3
...
class Thread1 extends Thread {
  ...
  // Construtor
  Thread1 (int id, int delayTime, LE m) {
    this.id = id;
...
class Thread2 extends Thread {
  ...
  // Construtor
  Thread2 (int id, int delayTime, LE m) {
    this.id = id;
    this.delay = delayTime;
    this.monitor = m;
  }
...
class Thread3 extends Thread {
  ...
  // Construtor
  Thread3 (int id, int delayTime, LE m) {
    this.id = id;
    this.delay = delayTime;
    this.monitor = m;
  }
...
class LeitorEscritor {
  ...
  public static void main (String[] args) {
    ...
    Thread1[] l = new Thread1[L]; // Threads leitores
    Thread2[] e = new Thread2[E]; // Threads escritores
    Thread3[] le = new Thread3[LE]; // Threads leitores e escritores
    ...
    for (i=0; i<L; i++) {
       l[i] = new Thread1(i+1, (i+1)*500, monitor);
       l[i].start(); 
    }

    for (i=0; i<E; i++) {
       e[i] = new Thread2(i+1, (i+1)*500, monitor);
       e[i].start(); 
    }

    for (i=0; i<LE; i++) {
      le[i] = new Thread3(i+1, (i+1)*500, monitor);
      le[i].start();
    }
  }
}
